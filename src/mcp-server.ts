...
async function checkAndSweepFunds() {
  return withSweepLock(async () => {
    ...
    const minThreshold =
      parseEther("0.000005") + totalFee + safetyBuffer;
    ...
    if (balance >= minThreshold) {
      ...
      if (valueToSend <= 0n) {
        console.error(
          "[Aegis] ⚠️ Balance too low after safety buffer. Skipping sweep.",
        );
        return globalTxHash;
      }

      console.error(
        `[Aegis] 💰 Deposit detected! Sweeping ${formatEther(valueToSend)} to Aegis... (Buffer: ${formatEther(safetyBuffer)})`,
      );
